...
def pick_best_model(training_results_path="models/training_results.vfinal-final.v2.json"):
...
def list_available_models(training_results_path="models/training_results.vfinal-final.v2.json"):
    models = {}
    best_name, best_path = pick_best_model(training_results_path)
    if best_name:
        models["Best (auto)"] = (best_name, best_path)

    for weights_file in sorted(Path("models").glob("resume_ner_model_*.pth")):
