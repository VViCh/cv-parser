...
log_file = "training_log.vfinal-final.v2.txt"
logging.basicConfig(
    level=logging.INFO,
    ...
    try:
        ...
        for epoch in range(EPOCHS):
            ...
            if improved:
                ...
                patience_counter += 1
                if val_loss > best_val_loss + VAL_LOSS_TOL:
                    loss_worse_count += 1
                else:
                    loss_worse_count = 0
                logger.info(f"No improvement ({patience_counter}/{PATIENCE})")

            if loss_worse_count >= OVERFIT_PATIENCE:
                logger.info("Early stopping triggered due to rising val loss (overfitting).")
                break
...
with open("training_results.vfinal-final.v2.json", 'w') as f:
    json.dump(results, f, indent=4)

logger.info("\nDetailed results saved to training_results.vfinal-final.v2.json")
